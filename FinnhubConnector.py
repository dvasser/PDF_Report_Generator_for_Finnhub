...
class FinnhubConnector:
    ...
    def look_up_stock(self, search_query: str) -> pd.DataFrame:
        ...
        search_results = pd.DataFrame(requests.get(f'{self.base_api_url}search?q={search_query}&token={self.api_key}').json())
        ...
        search_results.rename(columns= {'description':'Description', 'displaySymbol':'Symbol', 'type':'Type'}, inplace = True)
    ...
    def get_company_news(self, symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        df = pd.DataFrame(requests.get(f'{self.base_api_url}company-news?symbol={symbol}&from={start_date}&to={end_date}&token={self.api_key}').json())
            
        #return value error if the API call returns an empty data frame
        try:
            df.set_index(df['datetime'].apply(lambda x: datetime.utcfromtimestamp(x).strftime('%Y-%m-%d %H:%M')), inplace=True)
        except:
    ...
    def get_basic_financials(self, symbol: str) -> pd.DataFrame:
        past_year = pd.DataFrame(requests.get(f'{self.base_api_url}stock/metric?symbol={symbol}&metric=all&token={self.api_key}').json())
    ...
    def get_earnings_surprises(self, symbol: str) -> pd.DataFrame:
        ...
        #make the API call with proper parameters and create a data frame
        df = pd.DataFrame(requests.get(f'{self.base_api_url}stock/earnings?symbol={symbol}&token={self.api_key}').json())
        ...
        df.rename(columns= {'actual':'Actual','estimate':'Estimate','quarter':'Quarter',
                            'surprise':'Surprise','surprisePercent':'Surprise percent', 'symbol': 'Symbol','year':'Year'}, inplace=True)
    ...
    def get_current_quote(self, symbol: str) -> pd.DataFrame:
        df = pd.DataFrame(requests.get(f'{self.base_api_url}quote?symbol={symbol}&token={self.api_key}').json(), index=['Value'])
    ...
    def get_stock_candles(self, symbol: str, resolution: str, date_from: str, date_to: str, time_from='00:00:00', time_to='00:00:00') -> pd.DataFrame:
        
        #make the API call with proper parameters and create a data frame
        try:
            df = pd.DataFrame(requests.get(f'{self.base_api_url}stock/candle?symbol={symbol}&resolution={resolution}&from={self.convert_to_unix(date_from, time_from)}&to={self.convert_to_unix(date_to, time_to)}&token={self.api_key}').json())
        except:
    ...
    def get_crypto_symbols(self, exchange: str) -> pd.DataFrame:
        ...
        crypto_symbols = pd.DataFrame(requests.get(f'{self.base_api_url}crypto/symbol?exchange={exchange}&token={self.api_key}').json())
        
        if len(crypto_symbols) == 0:
            raise ValueError(f'{exchange} IS NOT A VALID EXCHANGE')
            
        crypto_symbols.rename(columns= {'description':'Description', 'displaySymbol':'Display symbol', 'symbol': 'Symbol'}, inplace= True)
        return crypto_symbols

    def get_crypto_candles(self, symbol: str, resolution: str, date_from: str, date_to: str, time_from='00:00:00', time_to='00:00:00') -> pd.DataFrame:
        ...
        #make the API call with proper parameters and create a data frame
        try:
            df = pd.DataFrame(requests.get(f'{self.base_api_url}crypto/candle?symbol={symbol}&resolution={resolution}&from={self.convert_to_unix(date_from, time_from)}&to={self.convert_to_unix(date_to, time_to)}&token={self.api_key}').json())
        except:
            raise ValueError(f"THERE IS NO DATA FOR-> {symbol} FROM {date_from} {time_from} TO {date_to} {time_to}")
    ...
    def stream_websocket(self, symbol: str) -> pd.DataFrame:
        ...
        #Allows event loops to be nested and wait for the result in Jupyter Notebooks.
        nest_asyncio.apply()
